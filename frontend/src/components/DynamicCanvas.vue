<template>
  <div class="ambient-mesh-root fixed inset-0 overflow-hidden pointer-events-none z-[-10]">
    <div class="low-res-viewport" :class="{ 'is-paused': !player.isPlaying.value }">
...
.blob-main {
  ...
  opacity: 0.65;
  animation: orbit-prime-1 17s infinite alternate cubic-bezier(0.45, 0.05, 0.55, 0.95);
}
...
@keyframes orbit-prime-1 {
  ...
  50%  { transform: translate3d(45%, 25%, 0) scale(1.35, 0.85) rotate(140deg); }
  100% { transform: translate3d(15%, 60%, 0) scale(0.9, 1.2) rotate(290deg); }
}

@keyframes orbit-prime-2 {
  0%   { transform: translate3d(0, 0, 0) scale(1); }
  100% { transform: translate3d(-55%, -45%, 0) scale(1.4) rotate(-200deg); }
}

@keyframes pulse-prime-3 {
  0%   { transform: translate3d(0, 0, 0) scale(0.85); }
  100% { transform: translate3d(-15%, 20%, 0) scale(1.35); }
}
...
.low-res-viewport.is-paused .blob-main   { animation-duration: 170s; }
.low-res-viewport.is-paused .blob-accent { animation-duration: 230s; }
