<template>
  <div
    ...
    :class="
      layout.isQueueOpen.value
        ? 'translate-x-0'
        : 'translate-x-full'
    "
  >
    <div class="flex items-center justify-between px-6 py-4 border-b border-black/5 dark:border-white/5 shrink-0 lg:mt-6">
    ...
    <div class="flex-1 overflow-y-auto min-h-0 px-2 py-4">
      ...
      <div v-if="player.displayQueue.value.length > 0" class="mb-8">
        <div class="flex items-center justify-between px-4 mb-3">
          <h3 class="text-sm font-semibold tracking-wider text-base-content/50 uppercase">Next in Queue</h3>
          <button class="text-xs font-medium text-base-content/50 hover:text-base-content" @click="player.clearQueue()">Clear</button>
        </div>
        ...
        <div class="space-y-1">
          ...
          >
            <div class="relative h-10 w-10 shrink-0 overflow-hidden rounded bg-base-300">
              <img :src="track.cover" class="h-full w-full object-cover" />
            </div>
            <div class="min-w-0 flex-1">
              <p class="truncate text-sm font-medium text-base-content">{{ track.title }}</p>
              <p class="truncate text-xs text-base-content/60">{{ track.artist }}</p>
            </div>
            <button
      ...
      <div v-if="upcomingPlaylist.length > 0">
        <div class="px-4 mb-3">
          <h3 class="text-sm font-semibold tracking-wider text-base-content/50 uppercase">Next from Playlist</h3>
        </div>
      ...
      <div v-if="player.displayQueue.value.length === 0 && upcomingPlaylist.length === 0" class="flex flex-col items-center justify-center h-40 text-base-content/40">
        <Icon icon="clarity:music-note-line" class="h-12 w-12 mb-3 opacity-50" />
...
const upcomingPlaylist = computed(() => {
  if (player.playlist.value.length === 0 || player.currentIndex.value < 0) return []
  ...
  if (player.shuffle.value) {
    ...
    for (let i = start; i < Math.min(player.playlist.value.length, start + 20); i++) {
      upcoming.push({
