<template>
  <aside
    ...
    :class="
      layout.isRightSidebarOpen.value
        ? 'w-[280px] px-4'
        : 'w-0 overflow-hidden px-0'
    "
  >
    <div
      class="flex items-center justify-between pb-6 pt-2"
      v-show="layout.isRightSidebarOpen.value"
    >
      <h2 class="text-sm font-bold tracking-tight text-base-content">
        Now Playing
      </h2>
      <button
    ...
    <div
      class="flex-1 overflow-y-auto hidden-scrollbar flex flex-col items-center text-center mt-4"
      v-if="layout.isRightSidebarOpen.value && currentTrack"
    >
      <div
        class="w-full aspect-square rounded-2xl overflow-hidden bg-base-200 shadow-md border border-white/10 mb-6"
      >
        <img
          v-if="!coverFailed"
      ...
      <div class="w-full px-2">
        <h3
          class="text-xl font-bold tracking-tight text-base-content line-clamp-2 leading-tight mb-2"
        >
          {{ currentTrack.title }}
        </h3>
      ...
      <div
        class="mt-8 w-full p-4 rounded-xl bg-black/5 dark:bg-white/5 border border-white/5 backdrop-blur-md"
      >
        <p
          class="text-xs font-semibold text-base-content/50 uppercase tracking-widest text-left mb-3"
        >
          Up Next
        </p>
        <div class="flex items-center gap-3 text-left">
          <div
            class="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-base-300"
          >
            <img
              v-if="nextTrack"
              :src="nextTrack.cover"
    ...
    <div
      v-else-if="layout.isRightSidebarOpen.value"
